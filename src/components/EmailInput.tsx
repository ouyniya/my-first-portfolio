...
export const EmailInput = ({ text }: {text: string}) => {
  ...
  return (
    <section className="pb-1 pt-3 pr-2">
      <div className="container">
        <div className="mx-auto w-full max-w-[400px] text-sm">
          <div className="relative">
            <input
              type="text"
              ref={inputRef}
              value={text}
              className="h-10 w-full rounded-lg border border-stroke bg-transparent py-3 pl-5 pr-14 text-dark outline-none duration-200 selection:bg-orange-300/25 focus:border-orange-300"
              readOnly
            />
            <button
              onClick={copyToClipboard}
              className="absolute right-0 top-0 flex h-10 w-14 items-center justify-center text-body-color duration-200 hover:text-white"
            >
              {copySuccess ? (
                <svg
                  width="20"
